...
def gdisconnect():
    # Only discconect a connected user.
    ...
    if result['status'] == '200':
        ...
        response = make_response(json.dumps('Successfully discconected.'), 200)
        response.headers['Content-Type'] = 'application/json'
        return response
    else:
...
@app.route('/sports/<int:sport_id>/catalog/JSON')
def sportCatalogJSON(sport_id):
...
# Specific item <catalog_id> JSON


@app.route('/sports/<int:sport_id>/catalog/<int:catalog_id>/JSON')
def catalogItemJSON(sport_id, catalog_id):
    catalogItem = session.query(CatalogItem).filter_by(id=catalog_id).one()
    return jsonify(CatalogItem=catalogItem.serialize)
    # Error: URL not found


# List of all sports and its ID JSON
@app.route('/sports/JSON')
def sportsJSON():
    sports = session.query(Sport).all()
    return jsonify(sports=[s.serialize for s in sports])
    # Error: 'Sport' object has no attribute 'serialize'


# Homepage: - Show all sports (and recently added items)
@app.route('/')
@app.route('/sports/')
def showSports():
...
def editSport(sport_id):
    ...
    if request.method == 'POST':
        ...
        session.add(editedSport)  # added
        session.commit()  # added
        flash("Sport %s Succesfully Edited" % (editedSport.name))
...
def newItem(sport_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('newItem.html', sport_id=sport_id, sport=sport)

# edit description or name for a sport item


@app.route('/sports/<int:sport_id>/catalog/<int:catalog_id>/edit/',
           methods=['GET', 'POST'])
def editItem(sport_id, catalog_id):
    if 'username' not in login_session:
        return redirect('/login')
    sport = session.query(Sport).filter_by(id=sport_id).one()
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash("Successfully edited sport item!")
        return redirect(url_for('showCatalog', sport_id=sport_id))
    else:
...
@app.route('/sports/<int:sport_id>/catalog/<int:catalog_id>/delete/',
           methods=['GET', 'POST'])
def deleteItem(sport_id, catalog_id):
    if 'username' not in login_session:
        return redirect('/login')
    sport = session.query(Sport).filter_by(id=sport_id).one()
    ...
    if request.method == 'POST':
        ...
        return render_template('deleteItem.html', sport_id=sport_id,
                               catalog_id=catalog_id, item=itemToDelete)
